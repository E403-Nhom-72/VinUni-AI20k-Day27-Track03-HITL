...
async def list_recent_threads(limit: int = 10) -> list[dict]:
    """Same query as `audit.replay.list_threads`, but returns rows instead of printing."""
...
with st.sidebar:
    ...
    recent = asyncio.run(list_recent_threads())
    if not recent:
        st.caption("No sessions yet — run a review below.")
    for r in recent:
        pr_short = r["pr_url"].rsplit("/", 1)[-1]
        label = f"PR #{pr_short}  ·  risk={r['worst_risk']}  ·  {r['events']} events"
        if st.button(label, key=f"thread_{r['thread_id']}", use_container_width=True):
...
def render_escalation_card(payload: dict) -> dict | None:
    ...
    with st.form("escalation"):
        answers: dict[str, str] = {
            q: st.text_input(q, key=f"escalation_q_{i}")
