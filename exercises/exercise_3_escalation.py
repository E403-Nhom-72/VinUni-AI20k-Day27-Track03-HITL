...
ANALYZE_SYSTEM_PROMPT = (
    ...
    "file/section of the diff it's about (e.g. 'tasktrack/storage.py: Storage.add builds SQL via "
    "string formatting — was this intentional, or should it use parameterized queries?'). Do not "
    "ask generic questions like 'What is the intent of this PR?'."
)
...
def node_synthesize(state: ReviewState) -> dict:
    ...
    with console.status("[dim]LLM refining review with reviewer answers...[/dim]"):
        refined: PRAnalysis = llm.invoke([
            {"role": "system", "content": ANALYZE_SYSTEM_PROMPT + (
                "\n\nYou already produced an initial analysis and asked the reviewer "
                "clarifying questions. They have answered — use their answers to resolve "
                "the uncertainty and produce a refined, more confident review."
            )},
            {"role": "user", "content": (
                f"Title: {state['pr_title']}\nDiff:\n{state['pr_diff']}\n\n"
                f"Initial summary: {a.summary}\n"
                f"Initial confidence: {a.confidence:.0%} ({a.confidence_reasoning})\n\n"
                f"Reviewer Q&A:\n{qa}"
            )},
        ])
